...
def main():
    ...
    if INPUT_FILES.exists():
        try:
            ...
            print(f"⚠ Lỗi đọc file {INPUT_FILES}: {e}")

    all_groups = []
    for source_key in ["topcv_jobs", "vnwork_jobs"]:
        groups = data.get(source_key, [])
        if isinstance(groups, list):
            all_groups.extend(groups)
        else:
            print(f"⚠ Dữ liệu không hợp lệ: '{source_key}' không phải list")

    if not all_groups:
        print("⚠ Không có dữ liệu job hợp lệ trong file đầu vào")
        return
